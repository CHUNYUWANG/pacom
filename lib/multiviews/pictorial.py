    ...
    for node in skeleton_sorted_by_level:
        ...
        if len(node['children']) == 0:
            ...
            for i in range(nbins):
                maxv_of_each_child = []
                maxi_of_each_child = []
                energy_at_i = unary[node['idx']][i]
                for child in children:
                    child_energy = states_of_all_joints[child][
                        'Energy'].squeeze()
                    pairwise_term = pairwise[(node['idx'], child)][i]
                    if type(pairwise_term) == scipy.sparse.csr.csr_matrix:
                        pairwise_term = pairwise_term.toarray()
                    values = child_energy * pairwise_term.squeeze()
                    maxv_of_each_child.append(np.max(values))
                    maxi_of_each_child.append(np.argmax(values))

                energy.append(energy_at_i * np.prod(maxv_of_each_child))
                children_state.append(maxi_of_each_child)

        res = {'Energy': np.array(energy), 'State': children_state}
